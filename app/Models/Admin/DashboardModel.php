...
{
    public function fetchCategoryListModel($postData){
        $fieldsArray = array('category_id','category_name','active','create_date');
        $draw = $postData['draw'];
        ...
        $searchQuery = "";
        if($searchValue!==''){
            $searchQuery  = '(category_name like "%' .$searchValue .'%" or
                            DATE_FORMAT(create_date,"%d-%m-%Y") like "%'.$searchValue.'%")';
        }
        $builder = $this->db->table('category_table');
        ...
        foreach($records as $row){
            $checked = !empty($row->active) ? 'checked' : '';
            $data = [];
            $data[] = '<a class="btn btn-sm btn-info" data-category-id="'.$row->category_id.'" title="Edit" href="'.base_url().'admin/add-category/'.$row->category_id.'"><i class="bx bxs-pencil text-white"></i></a>';
            $data[] = $row->category_name;
             $data[] = '<div class="form-check form-switch text-center">
                        <input class="form-check-input flexSwitchCheckChecked" type="checkbox" role="switch"  '.$checked.' data-category-id ="'.$row->category_id.'">
                    </div>';
            $data[] = '<a class="btn btn-sm btn-danger deleteButton" data-category-id="'.$row->category_id.'" title="Delete"><i class="bx bx-trash text-white"></i></a>';
            $finalData[] = $data;
        }
        $response = array(
            'draw'=>intval($draw),
    ...
    public function fetchStudentListModel($postData){
        ...
        foreach($records as $row){
            ...
            $data[] = !empty($row->create_date) ? $row->create_date : 'N/A';
            $finalData[] = $data;
        }
        $response = array(
            'draw'=>intval($draw),
    ...
    public function fetchedPaperListModel($postData){
        ...
        foreach($records as $row){
            ...
            $data[] = $row->create_date;
            $data[] = !empty($row->schedule_date) ? date('d-m-Y H:i',strtotime($row->schedule_date)) : '';
    ...
    public function fetchPurchaseHistoryModel($student_id){
